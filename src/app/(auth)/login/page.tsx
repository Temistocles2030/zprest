      ...
      {reqs.map(r => (
        <li
          ...
          className={`flex items-center gap-1.5 text-xs transition ${
            r.ok ? "text-green-400" : "text-white/35"
          }`}
        >
...
function LoginContent() {
  ...
  const handleRegistroOtp = useCallback(async (code: string) => {
    ...
    try {
      ...
      if (!verifyData.ok) {
        setOtpError(verifyData.error ?? "Código incorrecto");
        return;
      }
      ...
      if (!regResult.ok) {
        setOtpError(regResult.error ?? "Error al crear la cuenta");
        return;
      }
      ...
      if (signInError) {
        setOtpError("Cuenta creada, pero no se pudo iniciar sesión automáticamente");
        return;
      }
  ...
  const handleResetNuevaPassword = async () => {
    ...
    if (!resetOtpVerificado) {
      setError("Verificación no completada");
      return;
    }
  ...
  const INPUT =
    "w-full rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-sm text-white placeholder-white/30 outline-none transition focus:border-yellow-400/60 focus:ring-2 focus:ring-yellow-400/10";
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#060d1f] px-4">
        <Link href="/" className="mb-10 flex flex-col items-center gap-0">
          ...
          <span className="text-[12px] font-bold tracking-[0.18em] text-white/30 transition-colors hover:text-[#D4AF37]">
            IS45.123
          </span>
        </Link>
          ...
          <div className="mb-6 w-full max-w-sm rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-300">
            {errorParam === "eliminado" ? (
              <>
                ...
                <a
                  href="mailto:seguridad@zprest.com.ar"
        ...
        <div className="w-full max-w-sm space-y-4">
          ...
          {IS_MOCK && (
            <button
              ...
              className="w-full rounded-xl border border-yellow-400/30 py-2.5 text-xs font-medium text-yellow-400 hover:bg-yellow-400/5"
            >
            ...
            <div>
              <div className="mb-1 flex items-center justify-between">
                <label className={LABEL}>
                  CUIL/CUIT <span className="text-red-400">*</span>
                </label>
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#060d1f] px-4">
        ...
        <div className="w-full max-w-sm space-y-5 rounded-2xl border border-white/10 bg-white/5 p-8">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              onClick={() => setView("registro-paso2")}
              className="flex-1 rounded-xl border border-white/15 py-2.5 text-sm text-white/40 hover:text-white/70 transition"
            >
              ← Volver
            </button>
          </div>
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#060d1f] px-4">
        ...
        <div className="w-full max-w-sm space-y-5 rounded-2xl border border-white/10 bg-white/5 p-8">
          ...
          <button
            onClick={() => setView("registro-paso1")}
            className="w-full text-sm text-white/40 hover:text-white/70"
          >
            ← Paso anterior
          </button>
